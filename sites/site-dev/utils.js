...
export function usePageData(data, base = {}) {
  const result = { ...base, ...data }
  result._data = result
  return result
}

export function injectDataToPages(pages, data = {}) {
  for (const page of pages) {
    page.data = usePageData(data, page.data)
  }
}

export function findPages(baseDir, root) {
  const pages = []
  ...
  function walk(currentDir) {
    ...
    for (const ent of items) {
      ...
      if (ent.isDirectory()) {
        ...
        const entry = fs.existsSync(entryPath) ? entryPath.replace(root, '') : undefined
        pages.push({ name, filename, template, ...(entry && { entry }), data: usePageData({ filename }) })
      }
    }
  }
...
export function decoratePagesByLangs(pages, langPack, langs) {
  ...
  if (isMultiLang) {
    ...
    pages.push({
      ...
      data: usePageData({ ...originalPages[0].data, langs, filename: 'index.html' }),
    })
  }
